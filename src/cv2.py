...
def load_data(filename):
    dataset=pd.read_csv(filename,sep=',?\s*',header=None,index_col=None)
    cv=CrossValidation(dataset.values,5,shuffle_seed=666,split_seed=888)
    data=[x for x in cv]
    ...
    for i in range(len(data)):
        ...
        ret.append([train[:,:-1],train[:,-1],valid[:,:-1],valid[:,-1],test[:,:-1],test[:,-1]])
    return ret
...
def ORARS(train_X,train_y,valid_X,valid_y,test_X,test_y):
    org_config={
